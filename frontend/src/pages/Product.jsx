...
export default function Product() {
  ...
  return (
    <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
      ...
      <div className="flex gap-12 sm:gap-12 flex-col sm:flex-row">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1 mt-2">
            {Array.from({ length: 4 }, () => null).map((_, ind) => (
              <img key={ind} src={assets.star_icon} alt="" className="w-3.5" />
            ))}
            ...
            <div className="flex gap-2">
              {productData.sizes.map((item, index) => (
                <button
                  ...
                  className={`border bg-gray-100 ${item == size ? "border-orange-500" : "border-none"}   cursor-pointer py-2 px-4`}
                >
                  {item}
                </button>
              ))}
      ...
      <div className="mt-20">
        <div className="flex">
          <b className="border border-gray-300 p-3 text-sm">Description</b>
          <p className="border border-gray-300 p-3 text-sm">Reviews (122)</p>
        </div>
